/**
 * Un-confirms する where it was confirmed as vocabulary directly off a に
 * that marks the immediately preceding word — the "Nに+する" construction
 * (坊主頭にする "shave one's head", ことにする "decide to", ようにする "make
 * sure to", …). `isNiMarkedSuruConstruction`
 * (src/lib/vocabularySuggestions.ts) now excludes this from default-checked
 * — the meaning comes from the N+に phrase, not する itself, so a
 * standalone card for する tests nothing that phrase's own card doesn't
 * already cover — but `refreshVocabularySuggestionDefaults` deliberately
 * never reaches once a real selection already exists (see docs/STATUS.md,
 * 2026-09-24). This is the follow-up for the sentence(s) where it *was*
 * already confirmed before that fix — found via
 * scripts/diagnose-ni-suru-construction-confirmed.ts (superseded by this
 * script's own dry-run output). Same shape as
 ...
 *
 * Usage: npx tsx scripts/unlink-ni-suru-construction-confirmed.ts [--apply]
 */
