...
- [~] **Phase 2 — Existing data migration.** WaniKani kanji catalog importer run against production (2101 rows). JMDict scoped down to a local lookup tool (`npm run jmdict:lookup`) — bulk-populating `vocabulary_items` deferred to Phase 5 (see STATUS.md). One-time `anki_headless`-mediated import of Satori/Shadowing Anki notes → `Sentence`/`sentence_vocabulary` built and tested (`npm run import:anki-sentences`), not yet run against production.
