...
| A5 | Shadowing bridge (computed): a close **Analyze** result on a sentence with an existing `reading_in_context` card logs a natural encounter | `/books/:id/shadow/:sentenceId` (vocab proficient) → record a good take → **Analyze** → wait for it to settle | ⬜ | When timing + pitch both come back low, a muted "Close to the reference — logged as a natural encounter" line appears in the panel; `/study-items/<that card>` shows a new `natural_encounter` review + pushed-out due date. Re-analyze / analyze another take → no duplicate (dedupe). |
| A5b | Shadowing bridge (manual fallback): "Better"/"Same" A/B rating does the same when the alignment service is off | Same, but don't Analyze — just rate the take "Better" | ⬜ | Muted "Counted as a natural encounter…" line under "Past attempts". Deduped against A5. |
| A6 | Shadowing bridge no-ops when there's no `reading_in_context` card | Shadow + rate/analyze a sentence with no sentence card yet | ⬜ | No muted line; ☰ → Study items shows no new `sentence` card was created. |
...
| A8 | AnalysisPanel: reference + learner pitch contours share a time axis and (semitones mode) a y-axis | Analyze any attempt → look at "Reference pitch" / "Learner pitch" | ⬜ | Both contours fill the width from start-of-speech to end-of-speech and roughly line up (kana rulers underneath match). Reference no longer squashed into the left ~40%. In "Speaker-normalized" mode a flatter take looks flatter than the reference, not stretched to the same height; "Hertz" mode keeps each to its own range. |
