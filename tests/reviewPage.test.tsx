...
describe('ReviewPage', () => {
  ...
  it('seeds a pitch-accent card for a godan negative occurrence, graded against the conjugated reading', async () => {
    await seedBookWithSentence();
    const db = getDb();
    const now = new Date().toISOString();
    await suppressUnconditionalSentenceActivityTypes('sent-1');
    await addReferenceAudio('sent-1');
    // 走る [2] is accented; its negative 走らない gets a downstep right
    // before ない (Wiktionary Module:ja-acc-table, pitchAccentShift.ts) —
    // 5 morae (は/し/ら/な/い), drop after mora 3, one mora past the
    // citation stem boundary, not at it.
    await db.sentences.update('sent-1', { japanese: '公園を走らない。' });
    ...
    // 走らない is also a valid sentence_transformation candidate (conjugable
    ...
    await screen.findByText(/Listen, then mark where it falls/);
    // The reading line and mora choices must reflect the conjugated はしらない,
  ...
  it("frames a grammar_completion card with its reading-order passage context", async () => {
    const db = getDb();
    const now = new Date().toISOString();
    await db.books.add({
      id: 'book-grammar',
    ...
    await db.sentences.bulkAdd([
      {
        id: 'sent-grammar-before',
        normalizedKey: 'sent-grammar-before',
        japanese: '朝ごはんを食べました。',
        readingOnly: '',
        inlineReading: '',
        translation: 'I ate breakfast.',
      ...
      {
        id: 'sent-grammar-target',
        normalizedKey: 'sent-grammar-target',
        japanese: '忘れるわけがない。',
        readingOnly: '',
        inlineReading: '',
        translation: "There's no way I'd forget.",
      ...
      {
        id: 'sent-grammar-after',
        normalizedKey: 'sent-grammar-after',
        japanese: 'それから出かけました。',
        readingOnly: '',
        inlineReading: '',
        translation: 'Then I went out.',
    ...
    await db.bookSentences.bulkAdd([
      {
        id: 'bs-grammar-before',
        bookId: 'book-grammar',
        sentenceId: 'sent-grammar-before',
        position: 0,
        status: 'unstarted',
        addedAt: now,
      },
      {
        id: 'bs-grammar-target',
        bookId: 'book-grammar',
        sentenceId: 'sent-grammar-target',
        position: 1,
        status: 'unstarted',
        addedAt: now,
      },
      {
        id: 'bs-grammar-after',
        bookId: 'book-grammar',
        sentenceId: 'sent-grammar-after',
        position: 2,
        status: 'unstarted',
        addedAt: now,
      },
    ]);
    await suppressUnconditionalSentenceActivityTypes('sent-grammar-target');
    await confirmSentenceVocabulary('sent-grammar-target', []);
  ...
  it("is gated the same as vocabulary — surfaces even when the sentence's other vocabulary isn't proficient", async () => {
    ...
    const now = new Date().toISOString();
    await db.sentences.add({
      id: 'sent-grammar-loose',
      normalizedKey: 'sent-grammar-loose',
      japanese: '忘れるわけがない。',
      readingOnly: '',
      inlineReading: '',
      translation: "There's no way I'd forget.",
    ...
    // Deliberately no confirmSentenceVocabulary call — this sentence's own
    // vocabulary is unreviewed. The old gate (getSentenceFullReviewReadiness)
    // would have withheld the pattern entirely; the new one only needs a
